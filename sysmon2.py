...
def get_cpu_mobo_temperature():
    ...
    try:
        ...
        cpu_temp = psutil.sensors_temperatures().get('sch5627')[5].current
        # works on thinkpad mainboards
        # mobo_temp = psutil.sensors_temperatures().get('thinkpad')[0].current
        mobo_temp = psutil.sensors_temperatures().get('mt7921_phy1')[0].current
...
def get_time():
    """ Retrieve current time information.
...
def main():
    ...
    sysinfo_functions = [
        get_cpu_usage,
    #    get_load_avg,       #mee not need
    #    get_memory_usage,   #mee not need
    #    get_uptime,         #mee not need
